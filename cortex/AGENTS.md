...
- **Lifecycle:** `sweep.rs` (+`sweep/`), `proposals.rs` (+`proposals/`: the open-vocabulary tag-candidate reader and aggregator - reads borg's staged `distilled.yml` READ-ONLY for raw pre-filter candidates, reconciles trace-to-note identity through four tiers over borg's receipts DB read-only, and unions that arm with the note-derived one into `tag-proposals.yml`, and owns `promote_tags` behind `sb cortex tag-promote`: a targeted textual insert into one group of `canonical-tags.yml`, dry-run by default, refusing the deployed copy under `--apply`), `summarize.rs` (+`summarize/`), `migrate.rs`, `state.rs`, `report.rs`, `schema_docs.rs` (+`schema_docs/`: `sb cortex schema` renders `system/schemas/{type,origin,status}-values.md` from `vault::schema` plus `tag-values.md` from `canonical-tags.yml`, all with block-form `tags`; any other `*-values.md` in that directory is owned by no spec and is reported `obsolete` by `--check` and deleted by `--render`; snapshot fixtures under `schema_docs/fixtures/`).

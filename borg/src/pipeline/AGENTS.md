...
3. **ActiveTraceGuard before the permit wait.** Each trace registers its id (Drop removes it on return/panic/cancel). The watchdog consults `is_trace_active()` before declaring an intake row orphaned.

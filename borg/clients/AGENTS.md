...
- **`extension/`** — WebExtension (Firefox/MV3). Popup (`popup.html`/`popup.js`) reads the active-tab URL and POSTs to `/ingest`. **The fetch does NOT set `keepalive: true`** — on snap Firefox 150.x a keepalive POST is silently dropped (the toolbar click produces no ingestion, no daemon log), and the popup awaits the response before closing so the focus-loss-abort case keepalive guarded against doesn't occur in practice. The original design doc was wrong on this; `popup.js` forbids re-adding it. Checks `res.ok` (fetch doesn't reject on HTTP errors). `options.html`/`options.js` configure the daemon endpoint (default `localhost:8181`). `ingest-schema.json` (generated from `IngestRequest` via schemars at build) is a web-accessible resource for validation.

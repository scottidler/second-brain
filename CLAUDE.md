...
`facet` reads JSONL transcripts under `~/.claude/projects/`, clusters turns into cross-session work-items, mines **dialog-slice gems** (multi-turn, four-part anatomy: task, context, interaction, review) via fabric (haiku/sonnet/opus tiers), and renders one fencepost-merged markdown note per work-item under `notes/facet/prisms/<slug>.md`. A separate narrate-pass (`sb facet narrate`) synthesises **narrative spectra** (Session Arc, Cross-Session Arc, evergreen mode rollups) into `notes/facet/spectra/<slug>.md` with a strict opus-side rejection gate. A non-destructive dream-pass (`sb facet dream`) surfaces semantic-duplicate / cross-reference / stale-spectrum / narrative-candidate proposals under `notes/facet/dreams/`. `sb facet present <slug>` formats a narrative as a slide-deck outline. The daemon (`sb facet daemon`) runs all four passes on configurable cadences. The ledger lives at `~/.local/share/sb/facet/state.db` (rusqlite); v2 schema is bootstrapped by `bin/migrate-facet-v2.sh` or auto-applied on first v2 tick. Config at `~/.config/sb/facet.yml`. v2 design: `docs/design/2026-05-26-facet-v2-gems-and-narrative-spectra.md`. v1 (legacy, kept during cutover via `sb facet harvest --v1`): `docs/design/2026-05-26-facet-judgment-harvester.md`.

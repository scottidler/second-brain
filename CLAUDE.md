...
The receipts DB is the **sole** durable failure store; the sidecar is the durable raw-input record (modern `sb borg replay` re-injects from the trace's `source` via the daemon, not from the sidecar). The legacy markdown bookkeeping (`borg-intake.md`, `borg-dlq.md`, `borg-dlq-archive.md`, `borg-orphans.md`) and the `sb borg intake` / `sb borg dlq` / `audit --invariant` verbs were removed (`docs/design/2026-06-03-receipts-log-legacy-markdown-excision.md`). `bin/migrate-receipts` backfills any pre-receipts-era rows from the markdown into the DB and `bin/migrate-receipts --prune-legacy` deletes the four vault markdown files; run both once per host after deploying this change.

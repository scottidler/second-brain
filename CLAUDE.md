...
second-brain/
  ...
  glean/       -- Claude Code session distiller (harvest JSONL transcripts, cluster into work-items, distill four-part chunks, dream pass) -- lib-only, consumed by sb
...
- **Patterns:** borg's Fabric patterns live at `~/.config/sb/patterns/` (source of truth in `borg/patterns/`). The L2 patterns are `distill-article.md`, `distill-repo.md`, `distill-thread.md`, `distill-video.md`, `distill-video-chunk.md`, `distill-video-reduce.md`. Glean ships its own patterns (`glean-classify.md`, `glean-distill.md`, `glean-dream-{dedup,xref,stale}.md`) at the same `~/.config/sb/patterns/` location with source of truth in `glean/patterns/`.
...
- **Binary name:** `sb` (one binary, subcommands `borg`/`cortex`/`oracle`/`glean`/`status`/`doctor`/`bootstrap`). The borg/cortex/oracle/glean crates are lib-only.

## Glean: Claude Code session distiller

`sb glean` mines Scott's own `~/.claude/projects/*.jsonl` session transcripts into per-work-item knowledge chunks. Two-tier pipeline plus a non-destructive consolidation pass:

- `sb glean harvest`: one sonnet fabric call per session, upserts a `SessionRecord` into `~/.local/share/sb/glean/state.db`. Idempotent on `jsonl_sha256`; re-runs without changed input are no-ops.
- `sb glean cluster`: rematerializes the `work_items` table. Three stages: orphans (`is_orphan = true`) become Singletons; sessions sharing a `design_doc_focus` form one DesignDoc work-item per (repo, doc path); remaining sessions go through agglomerative soft-cluster on `vault::embedding::embed_query` similarity (default `complete-link`; algorithm and threshold tunable in `~/.config/sb/glean.yml`).
- `sb glean distill [--work-item <key>]`: one opus fabric call per work-item. Renders a four-part chunk (task / context / interaction / review) to `notes/glean/<slug>.md`. Stable identity is `content_hash = sha256(sorted member session_uuids)`; the render path renames an existing chunk in place on slug churn rather than orphaning operator edits. Fencepost-merged on re-distill (`<!-- glean:fencepost-start -->` / `<!-- glean:fencepost-end -->`).
- `sb glean dream`: three consolidation detectors (dedup, xref, stale). Emits non-destructive proposals to `notes/glean-dreams/<kind>-<hash>.md` (content-addressed: re-running with no corpus change is a no-op). Operator approves by editing `status: proposed` -> `accepted`; no apply CLI yet.

The vault's existing `notes/facet/` output (from the killed v1/v2 effort on `failed-facet-v1`) is left in place; glean writes only to `notes/glean/` and `notes/glean-dreams/`. The Phase 0 recognition-test gate (hand-validate one chunk before any pipeline ships) is the explicit lesson from the prior failure.

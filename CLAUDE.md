...
`facet` reads JSONL transcripts under `~/.claude/projects/`, clusters turns into cross-session work-items, mines **dialog-slice gems** (multi-turn, four-part anatomy: task, context, interaction, review) via fabric (haiku/sonnet/opus tiers), and renders one fencepost-merged prism note per work-item under `notes/facet/prisms/<slug>.md`. A separate narrate-pass (`sb facet narrate`) synthesises **narrative spectra** (Session Arc, Cross-Session Arc) into `notes/facet/spectra/<slug>.md` with a strict opus-side rejection gate. A non-destructive dream-pass (`sb facet dream`) surfaces semantic-duplicate / cross-reference / stale-spectrum / narrative-candidate proposals under `notes/facet/dreams/`. `sb facet present <slug>` formats a narrative as a slide-deck outline. The daemon (`sb facet daemon`) runs all four passes on configurable cadences. The ledger lives at `~/.local/share/sb/facet/state.db` (rusqlite); the schema is applied idempotently on every open via `facet::ledger::schema::apply`. Config at `~/.config/sb/facet.yml`. Design doc: `docs/design/2026-05-26-facet-v2-gems-and-narrative-spectra.md` (the date-stamped filename is historical; the running code does not call itself v2).

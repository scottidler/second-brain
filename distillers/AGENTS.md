...
- `tags.rs` (+`tags/`): the shared closed-vocabulary tag classifier called by borg's `finalize_tags` and by cortex classify / `--retag`. `TagClassifier` with `Deterministic`, `FabricClosedVocab`, `ClassifierDev`, and `FakeTagClassifier`; `build`/`build_fallback` select by config. The trait is **synchronous** because cortex's `classify_note` is sync and has no tokio runtime, so the Fabric impl takes the sync `FabricRunner` port rather than the async `FabricCaller`. Recorded API response in `tags/fixtures/`, so no test touches the network.

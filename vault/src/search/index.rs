...
impl super::SearchIndex {
    ...
    fn index_one_inner(&self, note: &Note, mtime: i64) -> Result<IndexAction> {
        ...
        if exists {
            ...
            self.conn.execute(
                "UPDATE notes SET
                    title = ?2, note_type = ?3, origin = ?4, status = ?5,
                    date = ?6, tags = ?7, source = ?8, creator = ?9, body = ?10,
                    summary = ?11, claims = ?12, modified_at = ?13,
                    quality = ?14, classified = ?15, classified_by = ?16,
                    confidence = ?17, needs_review = ?18, duplicate_group = ?19,
                    cortex_repo_stars = ?20, cortex_repo_last_commit = ?21,
                    cortex_repo_primary_language = ?22,
                    cortex_video_duration_seconds = ?23, cortex_video_channel = ?24,
                    cortex_video_published_at = ?25,
                    cortex_thread_platform = ?26, cortex_thread_post_count = ?27,
                    cortex_thread_author = ?28,
                    pinned = ?29,
                    trace = ?30, ingested = ?31, trace_expires = ?32,
                    capture_note = ?33, repo = ?34, repos_touched = ?35,
                    superseded_by = ?36
                 WHERE path = ?1",
                params![
                    path_str.as_ref(),
            ...
            self.conn.execute(
                "INSERT INTO notes (
                    path, title, note_type, origin, status, date, tags,

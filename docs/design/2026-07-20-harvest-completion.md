...
- **2026-07-20 (Scott): repo hubs nest at `entities/repos/<org>/<repo>.md`**, superseding the flat `repo-<org>--<repo>` on-disk scheme that `repo_hub_slug` produced. The path mirrors `~/repos/<org>/<repo>` as real nested directories: `repos/` subdir, then `<org>/<repo>.md`, NO `repo-` prefix, NO `--` separator. Applies ONLY to `HubKind::Repo`; person/concept/source/tag hubs stay flat at `entities/<slug>.md`. Rationale: mirrors the on-disk repo layout; drops the separator-encoding entirely (`:`/`|` were rejected — Obsidian/mobile filename hostility). `repo_hub_slug` is RETAINED as the injective `entities`-table id (`repo-<org>--<repo>`, disjoint from bare-token kinds); nesting makes the on-disk path injective for free (`a/b-c` -> `entities/repos/a/b-c.md` vs `a-b/c` -> `entities/repos/a-b/c.md`). Wikilinks to a repo hub use the full vault-relative path target with an `<org>/<repo>` display alias (`[[entities/repos/<org>/<repo>|<org>/<repo>]]`), which resolves unconditionally and is collision-safe across orgs sharing a repo basename. Bundled fix: `hub --synthesize` no longer LLM-synthesizes a ZERO-member hub (it hallucinated "no member notes were provided" live); it preserves the stub body fail-safe instead. The graph repo-member edge destination is the same nested path, so resolve-or-skip wires it (the old flat/legacy path mismatch is what left the live hub memberless).

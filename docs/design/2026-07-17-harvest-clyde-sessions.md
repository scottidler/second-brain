...
**Date:** 2026-07-17
**Status:** Implemented (Phases 0-13; cross-repo bridging deferred on clyde's files-touched release) (supersedes `2026-07-03-sessions-to-vault-loop.md`)
...
**Amended:** 2026-07-19 - added `repo:`/`repos-touched:` frontmatter, the Repo Hubs and Subject Grouping section, Phases 9-13, and their acceptance criteria; folds in section 3 of `2026-07-18-harvest-knowledge-goals.md`. Field name corrected same day against clyde's actual `2026-07-19-files-touched-export.md` design (was a placeholder `related-repos:`; clyde's contract ships `repos-touched` - see Repo Hubs section). Contract-review pass same day (Opus reviewer, Codex sat out): corrected the `repo:` handling from re-derive/canonicalize to validate-and-store-verbatim (both `repo` and `repos-touched` arrive byte-identical from clyde's single `repo_slug`, so there is nothing to re-derive; `repo_slug` is a path parser and cannot be "mirrored" on a slug); decided the `repo_hub_slug` encoding (injective on the org/repo split; slugify-lossy on `.`/`_`, accepted) for the `/`-in-`<org>/<repo>` -> flat-filename problem (Phase 10); corrected the clyde build-state to fully-implemented-but-unmerged/untagged; noted `repo` is present-`null` not omitted; added two single-repo-slice scope caveats.
...
**Slug + validation (determinism-critical).** Both `repo:` and `repos-touched` arrive from clyde ALREADY in canonical `<org>/<repo>` form (one `repo_slug`, byte-identical) - second-brain does NOT re-derive; it validates the `<org>/<repo>` shape and stores verbatim (see Phase 9). Hub identity and filename both come from one `repo_hub_slug("<org>/<repo>") = repo-<slug(org)>--<slug(repo)>` (injective on the org/repo split - the `--` boundary can't collide because `slugify` never emits a double-hyphen; slugify-lossy on `.`/`_`, an accepted membership-only merge; see Phase 10), so a note's single `repo:` and a matching `repos-touched` entry mint the SAME hub by construction. A value failing shape validation skips the repo edge and logs loudly, but the note still publishes (fail safe on the edge, never discard the knowledge).

**Two scope caveats on the single-repo slice (deferred with bridging, stated so they aren't a surprise):** (1) a session whose cwd is NOT a repo but which touched repo files (ran `claude` from `~/` and edited `~/repos/scottidler/loopr/...`) exports `repo: null` + a populated `repos-touched`; until bridging ships it gets NO repo-hub membership, since the `repo-member` edge fires only on a non-null `repo:`. This is a bigger slice than "multi-repo" implies. (2) A note published BEFORE the files-touched release is immutable and the cheap-filter skips its metadata-only cursor bump, so it never gains `repos-touched` through the normal loop - the one-time approve-diffs LLM backfill is the SOLE path onto older notes; `repos-touched` populates via the plain pass-through only on notes published after the release.
...
**Model:** opus
- Add `repo` typed field to `vault::Frontmatter` (`frontmatter.rs:20-47`, today falls into the untyped `extra` map); add the multi-repo storage field `repos-touched:` (name matches clyde's export contract field exactly - a direct pass-through of clyde's `repos-touched`, never re-derived in second-brain; populated once clyde ships files-touched, decision A2); store both `repo:` and `repos-touched` entries VERBATIM - do NOT re-derive or canonicalize them. Both arrive already in clyde's canonical `<org>/<repo>` form from a SINGLE function: `repo` is `repo_slug(cwd)` (`clyde/sessions/src/db/query.rs:301`) and every `repos-touched` entry is `repo_slug(path)` (`query.rs:335`), so the two fields are byte-identical to each other by construction and there is nothing for second-brain to normalize. `repo_slug` (`clyde/session/src/scope.rs:84-93`) is a PATH parser that returns `None` on an already-`<org>/<repo>` string, so "mirror `repo_slug`" is the wrong instruction and `borg::github::extract_repo_slugs` (URL-text, case-folding, different truncation) is the wrong tool - neither is used here. Second-brain's only job is VALIDATION: assert the value is a well-formed `<org>/<repo>` (exactly one `/` splitting two non-empty components); a value that fails validation SKIPS the repo edge and logs loudly (WARN, naming the note + value) but STILL PUBLISHES the note - a malformed slug implies an upstream clyde bug, never a reason to discard distilled knowledge. Note `repo` is present-as-`null` (not omitted) when clyde finds no `~/repos/<org>/<repo>` anchor (`clyde/sessions/src/export.rs:126`, no `skip_serializing_if`), unlike `repos-touched`/`files-touched` which OMIT when NULL - the deserializer must expect present-null for `repo` and `git-branch`. **Three-state `repos-touched` (bridge decision rule, producer-emphasized):** the field has three states the bridge MUST NOT conflate - (a) OMITTED (key absent, mirroring `files-touched` omission exactly): the touched set is UNKNOWABLE (transcript reaped, never backfilled) -> no signal, never infer "touched nothing"; (b) `[]` (present, empty): parsed, but no touched path resolved under `~/repos/<org>/<repo>` (scratch dirs, `~/.claude/...`, `/tmp/...`) -> DEFINITIVELY no cross-repo bridge; (c) populated -> the canonical set to bridge on. Model it as `Option<Vec<String>>` (None = omitted/unknowable, `Some(vec![])` = parsed-but-no-repo-paths, `Some(xs)` = the set); a default-empty `Vec` that collapses None into `[]` is WRONG - it silently turns "unknowable" into "touched nothing" and would fabricate a definitive no-bridge from missing data. Add a `repo` column to the `notes` table via idempotent `ALTER TABLE ADD COLUMN` (`schema.rs:346-367`, the proven `ensure_trace_columns` pattern), bind it in the upsert (`index.rs:230-245`), carry it on `GraphNoteRow` + its SELECT (`search/graph.rs:83-91`, `:322`); amend the Phase 5 note renderer to write `repo:` from the export contract's `repo` field (already present in contract v1) and `repos-touched:` from the contract's `repos-touched` field once present.
- **Success criteria:** a well-formed `<org>/<repo>` value is stored verbatim (no transformation - the stored value equals the input byte-for-byte); a value with no `/`, more than one `/`, or an empty component skips the repo edge and logs a WARN naming the note, while the note itself still publishes (knowledge not discarded); a note whose single `repo:` and one of whose `repos-touched` entries name the same repo mint ONE hub, not two, because both run through the identical `repo_hub_slug` (Phase 10) - a paired test asserts both derivations land on the same hub file; the deserializer accepts `repo: null` (present-null, not omitted) and still publishes; migration idempotent on existing DBs; `GraphNoteRow` carries `repo` end to end; harvest re-run after this phase produces notes with `repo:` populated.

#### Phase 10: Repo hub kind + deterministic edge
**Model:** opus
- `HubKind::Repo` (`hub.rs:121-175`, today's set is `{Concept, Creator, Source, Tag}`), an explicit slot in the first-wins precedence chain (`hub.rs:120`); the `repo-member` edge (note `repo:` -> repo hub) - unconditional, fires for every note carrying a `repo:`, minting the hub if it doesn't exist. This is genuinely new routing logic, not a mirror of the conditional shared-tag -> hub path (`graph.rs:274-288`), which only fires for over-cap blanket tags against an already-existing hub.
- **`repo_hub_slug` (the slash problem, decided).** A repo value is `<org>/<repo>`, but hub files are flat `entities/<slug>.md` (`hub.rs:70-72`) and a `/` cannot go in the stem (it would nest a directory), while the generic `slugify` (`hub.rs:89-102`) collapses `/` -> `-` and COLLIDES two distinct repos (`a`+`b-c` and `a-b`+`c` both -> `a-b-c`). So repo hubs get a dedicated slug: `repo_hub_slug("<org>/<repo>") = format!("repo-{}--{}", slugify(org), slugify(repo))`, splitting on the single `/` (`repo_slug` emits exactly one). It is injective ON THE ORG/REPO SPLIT - the boundary is unambiguous because an existing `slugify` invariant guarantees no slugified component contains `--` or a boundary hyphen (it collapses runs of non-alphanumerics to a SINGLE `-` via the `prev_hyphen` guard, `hub.rs:96-99`, then `trim_matches('-')`, `:101`), so the double-hyphen can never be forged. This kills the original org/repo-split collision: `a`+`b-c` -> `repo-a--b-c`; `a-b`+`c` -> `repo-a-b--c` (distinct). It is NOT fully injective per-component: `slugify` also folds `.` and `_` to `-`, both legal and distinct in GitHub repo names, so two real repos differing ONLY by those characters collide into one hub - `org/.github` and `org/github` both -> `repo-org--github`; `x/next.js` and `x/next-js` both -> `repo-x--next-js`. Accepted, not fixed: vanishingly unlikely across one operator's repo set (no `.`/`_` twins in practice), the `repo:` frontmatter stays byte-truthful (only hub MEMBERSHIP merges), and it is the exact lossiness creator/source hubs already carry through the same `slugify`. Case-folding is likewise inherited and correct - GitHub names are case-insensitive, so `Scott/Loopr` and `scott/loopr` are one hub. The SAME `repo_hub_slug` is the single source of truth for BOTH the `repo-member` edge key AND the on-disk filename, so a note's `repo:` and a matching `repos-touched` entry cannot diverge (this is what makes Phase 9's paired test pass). A value that does not split into exactly two non-empty components is rejected by the Phase 9 validator before it ever reaches `repo_hub_slug`.
- **Success criteria:** a session note carrying `repo: scottidler/loopr` joins hub `entities/repo-scottidler--loopr.md` on every sweep (3f repo determinism); the adversarial pair `a/b-c` and `a-b/c` mint DISTINCT hubs (`repo-a--b-c` vs `repo-a-b--c`) - the frozen-corpus determinism fixture set includes a `/`-bearing repo (existing Concept/Creator/Source/Tag slugs are single-token and never exercise the slash path, so this fixture is mandatory); `Scott/Loopr` and `scott/loopr` fold to the one hub (case-insensitive); sweep twice on a frozen corpus yields identical repo groupings byte-for-byte (3f frozen-corpus determinism); a repo sharing a name with an existing concept does not collide (the `repo-` prefix + `--` boundary keep the namespaces disjoint).
...
- [ ] The checked-in 2026-07-02 golden fixture produces the DETERMINISTIC thread notes the selection gate + clustering rules yield on the real captured metadata (asserted ids, cluster count, note count), unattended. **Corrected 2026-07-20 against real clyde data (see Resolved Decisions):** the deterministic outcome for the 07-02 arc is **2 selected -> 1 note, 2 rejected**, NOT the hand-written summary's "4 sessions = 1 note". Two of the four arc sessions are `enrich-status: skipped-personal` with a non-repo cwd (`/home/saidler`) and are correctly rejected; the two work/repo survivors share `(cwd, branch)` and cluster to one note. The hand-note's "4" was a human cross-cwd SUBJECT grouping - exactly the view deferred to the repo-hub layer (Phases 9-13), not a selection-gate output
...
- [ ] A session note carrying `repo: scottidler/loopr` joins hub `entities/repo-scottidler--loopr.md` on every sweep; a note carrying repos X+Y (post-files-touched bridging) joins BOTH hubs deterministically; the pair `a/b-c` and `a-b/c` mint distinct hubs (`repo_hub_slug` injective on the org/repo split)
...
- [ ] A well-formed `<org>/<repo>` value is stored verbatim (never re-derived); a value with no `/`, more than one `/`, or an empty component skips the repo edge and logs a WARN while the note still publishes; a note's single `repo:` and a matching `repos-touched` entry mint ONE hub via the shared `repo_hub_slug`
...
- 2026-07-20 (Phase 3 implementation, real-data finding): the 07-02 golden-fixture premise ("4 sessions = 1 note") does NOT survive contact with the live catalog. The four named arc sessions export as: `9521f589` (29 msgs, cwd `/home/saidler`, `skipped-personal`, personal) and `4e55a52c` (389, same cwd, `skipped-personal`, personal) - both REJECTED (personal enrichment + non-repo cwd, two counts); `871f6428` (486, cwd `tatari-tv/slack-cli/main`, branch main, `ok`, work) and `4ae69e3a` (320, same cwd/branch, `ok`, work) - both SELECTED, ~15s apart, clustering to ONE note. Deterministic truth = **2 selected -> 1 note, 2 rejected**. This is the deterministic selection contract working AS DESIGNED, not a bug: the hand-written summary's "4" was a human cross-cwd subject grouping that crossed the selection bar, which is the repo-hub layer's job (Phases 9-13), never the selection gate's. Preserving the literal "4" would require harvesting personal/non-repo sessions, contradicting the Selection section and the reaffirmed "harvest must SELECT" principle. Golden fixture asserts the deterministic outcome; the divergence is documented with evidence in the Phase 3 implementation notes. Acceptance criterion #1 corrected accordingly.
...
- **Second cross-repo dependency (2026-07-19, Phases 9-13 only):** multi-repo bridging depends on clyde's files-touched contract addition (`clyde/docs/design/2026-07-19-files-touched-export.md`). As of 2026-07-19 clyde's build is FULLY IMPLEMENTED on branch `files-touched-export` but UNMERGED and UNTAGGED: all four phases are committed - parser extraction (`eb0923f`), catalog column + `SCHEMA_VERSION` 5->6 migration + `--reparse` backfill (`363a2f5`), export fields `files-touched`/`repos-touched` (`03418f6`), fixtures + contract-doc amendment (`701ed29`) - and the design doc is marked Implemented (`3e853bc`). The gate is therefore the release, not the code: second-brain's bridging consumption waits for clyde to MERGE and TAG that branch, then `cargo install`. Ship order stands: clyde (files-touched, tagged) -> second-brain (repo-hubs + bridging, Phases 9-13) -> obsidian vault (data, no code). The single-repo repo-hub slice (Phases 9-10) depends only on the `repo` field the contract already exposes (contract v1, shipped + released) and is NOT gated on the files-touched release. When that release lands, second-brain's contract fixture (Phase 0) MUST be captured from a real `clyde session export` payload, not hand-authored, so the seam is pinned to reality.
...
- Repo hubs (Phases 9-13): validation test (well-formed `<org>/<repo>` stored verbatim; a shape-invalid value skips the edge + logs, note still publishes); `repo_hub_slug` collision test (`a/b-c` vs `a-b/c` -> distinct hubs via the `--` boundary; case-fold merge `Scott/Loopr`==`scott/loopr`; and the ACCEPTED `.`/`_`-fold merge documented, e.g. `org/.github`==`org/github`) with a `/`-bearing frozen-corpus fixture; three-state `repos-touched` test (omitted deserializes to `None` and yields no bridge inference; `Some(vec![])` is distinct from `None` and also mints no bridge; `Some(xs)` bridges - asserting a default-empty `Vec` would collapse the first two); frozen-corpus determinism test (sweep twice -> identical groupings byte-for-byte); monotonic stability harness (assign, add N, re-run -> zero moves) extended to assert `apply_linking` is add-only; concept-recall eval set bounding the glossary/alias-coverage gap; synthesis-failure test (a forced `cortex hub --synthesize` failure leaves the prior body byte-identical).

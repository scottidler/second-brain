...
- Repo hubs (Phases 9-13): validation test (well-formed `<org>/<repo>` stored verbatim; a shape-invalid value skips the edge + logs, note still publishes); `repo_hub_slug` collision test (`a/b-c` vs `a-b/c` -> distinct hubs via the `--` boundary; case-fold merge `Scott/Loopr`==`scott/loopr`; and the ACCEPTED `.`/`_`-fold merge documented, e.g. `org/.github`==`org/github`) with a `/`-bearing frozen-corpus fixture; frozen-corpus determinism test (sweep twice -> identical groupings byte-for-byte); monotonic stability harness (assign, add N, re-run -> zero moves) extended to assert `apply_linking` is add-only; concept-recall eval set bounding the glossary/alias-coverage gap; synthesis-failure test (a forced `cortex hub --synthesize` failure leaves the prior body byte-identical).

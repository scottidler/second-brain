...
- **`repo_hub_slug` (the slash problem, decided).** A repo value is `<org>/<repo>`, but hub files are flat `entities/<slug>.md` (`hub.rs:70-72`) and a `/` cannot go in the stem (it would nest a directory), while the generic `slugify` (`hub.rs:89-102`) collapses `/` -> `-` and COLLIDES two distinct repos (`a`+`b-c` and `a-b`+`c` both -> `a-b-c`). So repo hubs get a dedicated, injective slug: `repo_hub_slug("<org>/<repo>") = format!("repo-{}--{}", slugify(org), slugify(repo))`, splitting on the single `/` (`repo_slug` emits exactly one). Injectivity rests on an existing `slugify` invariant: it collapses runs of non-alphanumerics to a SINGLE `-` and trims, so no slugified component ever contains `--`; the double-hyphen is therefore an unambiguous boundary that no component can forge. `a`+`b-c` -> `repo-a--b-c`; `a-b`+`c` -> `repo-a-b--c` (distinct). Case-folding is inherited from `slugify` (lowercase), which is correct - GitHub org/repo names are case-insensitive, so `Scott/Loopr` and `scott/loopr` are one repo and one hub. The SAME `repo_hub_slug` is the single source of truth for BOTH the `repo-member` edge key AND the on-disk filename, so a note's `repo:` and a matching `repos-touched` entry cannot diverge (this is what makes Phase 9's paired test pass). A value that does not split into exactly two non-empty components is rejected by the Phase 9 validator before it ever reaches `repo_hub_slug`.
...
- [ ] A session note carrying `repo: scottidler/loopr` joins hub `entities/repo-scottidler--loopr.md` on every sweep; a note carrying repos X+Y (post-files-touched bridging) joins BOTH hubs deterministically; the pair `a/b-c` and `a-b/c` mint distinct hubs (injective `repo_hub_slug`)
...
- [ ] A well-formed `<org>/<repo>` value is stored verbatim (never re-derived); a value with no `/`, more than one `/`, or an empty component skips the repo edge and logs a WARN while the note still publishes; a note's single `repo:` and a matching `repos-touched` entry mint ONE hub via the shared injective `repo_hub_slug`

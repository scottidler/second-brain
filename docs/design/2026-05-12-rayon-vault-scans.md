...
- [x] **Resolved (v0.5.42, Architect round 2):** `borg::migrate::run_migrate` (`borg/src/migrate.rs:10`) is now parallel. The initial deferral was anchored on the wrong precedent (`run_backfill_ingested`, which uses `write_atomic` with parent-dir fsync); Architect correctly pointed out `run_migrate` uses plain `std::fs::write` and shares the same lock-contention profile as `apply_quality`. Extracted `migrate_one_note(path, vault_root, apply, config) -> Result<Option<MigrateOutcome>>`, called from `par_iter().map().collect::<Result<Vec<_>>>()`, followed by a sequential drain that owns the deterministic `println!` order, the `changed_count` increment, and the `ledger_entries` accumulation. Console output is now batched at end-of-loop rather than streamed inline, which is acceptable for a one-shot maintenance command where the dry-run output is reviewed all at once.

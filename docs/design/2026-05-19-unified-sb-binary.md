...
- Create `systemd/borg.service` and `systemd/cortex.service` (keep the existing user-facing unit names; just change ExecStart paths). Use `%h` for the home-directory expansion: `ExecStart=%h/.cargo/bin/sb borg daemon --start` and `ExecStart=%h/.cargo/bin/sb cortex daemon --start`. Mirror the hardening directives from the current hand-authored units (`NoNewPrivileges`, `ProtectSystem=strict`, `ProtectHome=read-only`, `ReadWritePaths=%h/repos/scottidler/obsidian`, `PrivateTmp`). These shipped units are **base units only**: they do not contain the `ExecStartPre` secrets-decryption step that Scott's current `borg.service` uses (because that path is machine-specific). Per-machine secrets management lives in `~/.config/systemd/user/borg.service.d/secrets.conf` as a drop-in - systemd merges automatically. The drop-in is *not* checked into the repo; it stays a per-machine concern.
...
- Teach `otto deploy` (in `.otto.yml`) to sync `systemd/*.service` into `~/.config/systemd/user/` with `cp -f` (overwriting any base unit there) and run `systemctl --user daemon-reload`. The user's `*.service.d/*.conf` drop-ins are untouched because they're separate files in a sibling directory. Existing pattern-sync and shared-config-sync logic stays.

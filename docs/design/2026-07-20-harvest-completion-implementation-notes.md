...
### Design decisions
- Gap-filled the ONE nullable field Phase 0/1 never exercised: `BodyMessage.role`/`text` (defensive `Option<String>`, future-malformed tolerance on the `--with-body` path). Added `borg/src/harvest/contract/tests.rs::body_message_role_and_text_are_present_null_tolerant` (present-null AND wholly-omitted both deserialize to `None`) and `borg/src/harvest/watermark/tests.rs::canonical_body_text_degrades_gracefully_on_null_role_and_text` (the identity-hash render degrades to an empty string, never panics). Every other Phase-1-relaxed field (`cwd`/`created`/`title`/`first_prompt`) was already covered by `contract/tests.rs::parse_tolerates_null_string_fields` and `select/tests.rs::rejects_null_created`/`rejects_unparseable_created` - confirmed by revert-verification below, not re-tested.
- Added `borg/src/harvest/cluster/tests.rs::null_created_bypassing_selection_is_a_loud_backstop_error` - the SECOND layer of the created guard (`cluster.rs::parse_ts`'s fail-loud backstop for a null-`created` record that bypassed `select.rs`'s primary guard). Locks the doc's "Null `created` is rejected before clustering (not just parsed)" contract at both seams.
- Extended the Phase 4 monotonicity/determinism harness to the INCREMENTAL path (every real nightly `cortex graph`/daemon-tick sweep after the first full rebuild), which the Phase 4 tests never exercised (they only asserted full-rebuild snapshot equality):
  - `cortex/src/graph/tests.rs::repo_member_edges_are_monotonic_across_incremental_builds` - a note's repo-member edge from a full-rebuild sweep survives an INCREMENTAL sweep that grows the corpus with a new multi-repo note; the untouched note is never even reprocessed (`stats.notes_processed == 1`), so the surviving edge proves nothing removed it, not that it was rebuilt identically.
  - `cortex/src/linking/tests.rs::apply_linking_across_growing_sweeps_never_removes_prior_links` - extends the existing single-call add-only test (`apply_linking_is_add_only_never_removes_or_alters_content`) and the identical-vault two-pass convergence test (`two_consecutive_link_passes_converge_to_zero_writes`) to a THIRD sweep where the vault GROWS (a brand new note lands); the prior sweep's inserted wikilink is asserted byte-identical afterward.
- Added the standing `sb doctor` harvest drift guard (Opus SE K2), mirroring the `degraded_24h` pattern exactly:
  - `vault::receipts::ReceiptKind` gained no new variant; `borg::receipts::count_kind_since(conn, kind, since_iso)` (`borg/src/receipts.rs`) is the new counting primitive, windowed on `received_at` (not `terminal_at` like the failed/crashed/degraded counters) because the guard cares about "did the pipeline TOUCH a session at all", not whether it reached a terminal success.
  - `borg::triage::HarvestDriftStats` + `harvest_drift_stats()`/`harvest_drift_stats_at()` (`borg/src/triage.rs`) compute two signals: `timer_has_run` (the watermark cursor is `Some` - every LIVE run persists it via `apply_plan_to_state`/`state.save`, even a run that selected/published nothing, so it is independent of the receipts table) and `session_receipts_in_window` (count of `kind=session` receipts of ANY status in the last `HARVEST_DRIFT_WINDOW_DAYS` = 3 days). `HarvestDriftStats::should_warn()` fires only when BOTH: the timer has proven it can run, AND the recent window produced literally zero session activity - the exact structural signature of a FUTURE total-abort contract drift (which dies before even a rejected receipt gets written, so the existing failed/crashed/degraded counters stay silent too).
  - Wired into `sb/src/cli/checks.rs::borg_findings()` immediately after the `degraded_24h` block, same `Finding::warn` shape and suggested-fix pattern.
  - Test: `borg/src/triage/tests.rs::harvest_drift_warns_when_timer_has_run_but_zero_recent_session_receipts` simulates the exact zero-receipt-since-timer-run state (a saved `WatermarkState{cursor: Some(1500)}` + an in-memory receipts DB whose only session receipt is backdated outside the window) and asserts `should_warn() == true`; two companion tests (`harvest_drift_never_warns_before_the_timer_has_ever_run`, `harvest_drift_does_not_warn_when_recent_session_receipts_exist`) pin the two non-warning states so the guard cannot regress into a false-positive-on-every-fresh-install or false-positive-when-healthy shape.

### Deviations
- No test drives `borg_findings()` (the `Finding`-rendering function) end-to-end. It loads `borg::config::load_config::<Config>(None)` from the real `~/.config/sb/borg.yml` and opens the real receipts DB via `receipts::open_default()`, neither of which is redirectable without new XDG-env test infrastructure this crate does not have at that seam - the same documented gap the Phase 5 notes recorded for `register_systemd_units` (two sibling untested calls in the same function). The bite lives one seam lower, at `harvest_drift_stats_at`'s path/conn-injectable core, which is fully tested (three states: never-run, healthy, drifted). Same effect (the WARN logic is exercised and proven correct), correct seam (the one that is actually unit-testable offline).
- `HARVEST_DRIFT_WINDOW_DAYS = 3` is a hardcoded const, not a new `harvest.yml` config knob. The design doc's Phase 6 bullet says "over N days" without specifying a source; treated as a diagnostic threshold (like the pre-existing fixed 24h windows `degraded_24h`/`crashed_24h`/`failed_24h` already use), not a behavior-shaping config value per `general.md`'s "config drives behavior" carve-out for governance-rule *scope* (this is a doctor-check tuning constant, not a rule that runs/doesn't run).

### Tradeoffs
- Revert-verification was done ONCE per guard class rather than per individual field, since the four Phase-1-relaxed fields (`cwd`/`created`/`title`/`first_prompt`) are structurally identical (`#[serde(default)] Option<String>`) and asserted by the SAME test function (`parse_tolerates_null_string_fields`); reverting `cwd` alone already proved the class bites (see Revert-verification results below), and a compile error is a stronger, not weaker, signal than a per-field runtime RED (it blocks the whole crate, not just one test binary).
- The per-record-skip revert experiment needed a temporary `#[allow(dead_code)]` on `RawExport` to get PAST the `#![deny(dead_code)]` compile gate and observe the actual runtime test failures (rather than stopping at the dead-code error, which is itself a valid but less informative bite). Both the revert and the temporary allow were fully restored; `git diff` on `contract.rs`/`select.rs` is empty against the base commit.

### Revert-verification results (temporarily broken, confirmed RED, then restored - no revert left in the tree)
- **Removed the `Option` on `cwd`** (`borg/src/harvest/contract.rs`, `SessionRecord.cwd: String` instead of `Option<String>`): does not compile. 9 compile errors across `contract/tests.rs::parse_tolerates_null_string_fields` (direct cause: `assert_eq!(empty_session.cwd, None)` requires `Option`), `publish/tests.rs`, `select/tests.rs` (x2), and `pipeline/session/tests.rs` all construct/compare `cwd` as `Option<String>`. A compile failure blocks `otto ci` outright - a stronger bite than a runtime RED.
- **Removed the per-record parse skip** (`borg/src/harvest/contract.rs::parse_export`, reverted to one whole-batch `serde_json::from_slice::<SessionExport>`): `harvest::contract::tests::malformed_record_is_skipped_and_the_rest_parse` and `harvest::contract::tests::unreadable_session_id_falls_back_to_element_index` both went RED (panicked with the whole-batch serde error instead of the expected per-record skip + rejection).
- **Removed the `created` guard** (`borg/src/harvest/select.rs::evaluate_selection`, deleted the `match record.created.as_deref() { ... }` block): `harvest::select::tests::rejects_null_created` and `harvest::select::tests::rejects_unparseable_created` both went RED (`called Result::unwrap_err() on an Ok value`).

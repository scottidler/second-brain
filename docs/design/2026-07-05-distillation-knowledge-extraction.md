...
- **2026-07-05 (post-ship) - Claim embeddings are OFF by default; gated behind a `cortex.yml` toggle.** The Phase 9 retrieval gate FAILED on live measurement (see the live-gate checklist result in Open Questions): claim embeddings regressed nDCG (0.8795 → 0.8471) AND recall (0.7981 → 0.7852), so they displace precision without adding recall on this corpus - a direct violation of the recall-only invariant above. A second review panel (Architect/Gemini + Staff Engineer/Codex) unanimously recommended gating claim embedding behind a default-off config toggle, and independently proved the named kind-weighted-pooling contingency **cannot manufacture recall** (a distance penalty only makes claim rows *less* competitive; best case is a net-zero no-op, still failing the "add recall" half of the invariant). Disposition: (1) claim (and the other kinds) are gated behind a `cortex.yml` `embed.kinds` toggle, `claim` default **false** - this also fixes the latent defect that the daemon tick hardcoded claim embedding ON before the gate was run, making `--drop-kind claim` non-sticky; (2) `--kind claim` remains an explicit CLI override for a future guard-first pooling experiment; (3) kind-weighted pooling is demoted from "the fix" to an optional bounded experiment - re-enable only if a penalty sweep produces a genuinely positive per-query recall delta with no nDCG regression; (4) defect #6 ("claims invisible to default retrieval") is dispositioned as addressed via the existing `mode=bm25`/`hybrid` **manual** FTS fallback, NOT via default-pipeline embeddings - the eval shows claims carry no dense-retrieval signal the summary lacks on this corpus. BM25-default-on (Alternative 3) stays rejected/settled and was not reopened. (Author + second panel, confirmed.)
...
6. Phase 9 retrieval: `sb oracle eval` on `configured` before backfill (record per-query + aggregate baseline) → `sb cortex embed --backfill` → `sb oracle eval` after; accept only if no per-query nDCG regression beyond noise AND aggregate non-negative. On failure: `sb cortex embed --drop-kind claim`, implement the kind-weighted-pooling contingency (comment sited at the `search_vector` max-pool), re-backfill, re-measure.
   - **RESULT (2026-07-05): GATE FAILED.** Measured live on the daemon host (`configured` target, K=10, 9 queries, judge calibrated TRUSTWORTHY): without claims nDCG@10 **0.8795** / recall 0.7981 / MRR 1.0000; with claims **0.8471** / 0.7852 / 0.9444. All three metrics regressed and recall itself dropped, so claim embeddings are not merely failing to add recall - they displace relevant notes out of the top-10 via `search_vector`'s all-kind max-pool. This violates the recall-only invariant. See the Resolved Decision below for the disposition.

...
### Design decisions
- `init_vision_permits` wired in `borg::startup::init_permits` - `borg/src/startup.rs:init_permits` - this is the single sanctioned process-wide init site: it already sizes `GENERAL_PERMITS`/`HEAVY_PERMITS`, runs exactly once per borg process for BOTH the daemon and every CLI invocation (called from `sb`'s `BorgCli::run` before the subcommand match, `sb/src/cli/borg.rs:261`), and runs before any path that can reach `pipeline::process_content` - therefore before any vision call from `try_extract_slides` or the image-ingest `vision_extract`. Sizing the vision pool here (from `youtube.slides.content-filter.max-vision-concurrency`) is the natural peer of the existing pool sizing; no second init site, no lazy first-use guard needed. The pool's own `init_vision_permits` clamps `cap.max(1)`, so a misconfigured `0` cannot wedge the gate.
- Filter-enabled path is a separate orchestrator `classify_and_filter_slides` - `borg/src/pipeline/handlers.rs:classify_and_filter_slides` - `try_extract_slides` branches on `content_filter.enabled`: disabled keeps the byte-for-byte `segment_with_pairs` call; enabled runs the new path (pure prefix -> classify -> keep-filter -> materialize+OCR kept -> shape). The async/network step (classification) lives in the orchestrator, not in the pure `slides.rs` helpers.
- Pure prefix extracted to `slides::prepare_runs` - `borg/src/slides.rs:prepare_runs` - hash frames (rayon, mirroring `segment`) -> `cluster_frames` -> `collapse_runs` -> per-run `best_frame`. Keeps `handlers.rs` thin (no decode/hash logic in the orchestrator) and keeps the prefix pure and testable. No `drop_transitions` on this path, per the doc: a true transition is a short run that classifies title-card/other and the keep-filter removes it.
- Kept-run materialization in `slides::segment_filtered` - `borg/src/slides.rs:segment_filtered` - copies each kept best frame into `slides/slide-NNN.jpg`, OCRs the materialized copies (the SAME frame that is embedded, so image and OCR text always agree), binds transcript pairs, and computes shape via `shape_from_kept_count` (bypassing `propose_note_shape`'s ratio). `transitions_dropped` is recorded as 0 on this path; `compression_ratio` still uses `frames_after_mpdecimate` so it reflects the full extraction, not just survivors.
- `render_pattern_input` restricted to kept runs by construction - the filtered manifest contains only kept slides, so `render_pattern_input(&manifest)` is implicitly kept-only; no change to that function was needed and the dead `![]()` links are now irrelevant (no dropped slide reaches the pattern).
- Pure keep-filter as `classify::keep_outcome` + `KeepOutcome` enum - `borg/src/slides/classify.rs:keep_outcome` - one source of truth for the keep/drop decision, unit-tested directly. The orchestrator maps each result to a `KeepOutcome`, records it into a `ClassifyTally`, and only pushes `Keep` runs into the `KeptRun` set.
- Observability tally as `classify::ClassifyTally` - `borg/src/slides/classify.rs:ClassifyTally` - emitted once per ingest at INFO as a single structured line `slide-filter tally[<trace>]: classified=.. kept=.. dropped-low-confidence=.. dropped-not-in-keep=.. dropped-api-error=.. dropped-parse-error=..`. A nonzero `dropped-api-error` is the operator's "filtering degraded" signal (per `borg/AGENTS.md`'s `degraded_24h` philosophy), distinct from a legitimately image-free note (which classifies zero runs).
- Logging: `classify_and_filter_slides` and `prepare_runs`/`segment_filtered` log entry/exit at DEBUG with trace id and counts; the per-run outcome loop is TRACE; the tally is the one INFO line.

### Deviations
- Phase 3's `classify_slide`/`classify_slides` return type changed from `Result<SlideClass>` (eyre) to a typed `Result<SlideClass, ClassifyError>` with variants `Read`/`Api`/`Parse`/`Join`. Phase 4's required observability tally must distinguish `dropped-api-error` (a degradation signal) from `dropped-parse-error` (the model returning off-format text), and the typed-seam rule forbids re-parsing that distinction out of a flattened error string. The typed error is the clean way to carry the cause. `parse_classification` itself still returns eyre `Result<SlideClass>` (its tests are unchanged and pass); only the two async wrappers gained the typed error. No other caller consumed `classify_slides` (it was unwired before this phase), so blast radius is nil.

### Tradeoffs
- New `KeptRun` struct + `segment_filtered` builder vs reusing `materialize_slides`+`segment` by synthesizing `Cluster`s - chose the dedicated builder so the filter path expresses exactly its own shape rule (`shape_from_kept_count`, `transitions_dropped = 0`) without threading a "skip drop_transitions / skip propose_note_shape" flag through `segment`. Keeps both paths readable and independently testable; the small cost is one more builder function.
- Tally distinguishing api vs parse via a typed error (above) vs a single "dropped-error" bucket - chose the split because the doc's Phase 4 scope names both buckets explicitly and the api-error bucket is the operator's degradation signal; collapsing them would hide the actionable case.

...
| Free-tier quota (20,000 fast classifications per IP per day) blocks a tuning cycle | Certain on free tier | Med | One full retag is 5,440 calls (27% of a day), but a threshold or vocabulary iteration is several passes and the 2026-09-19 back-tests plus the 2026-09-20 due-diligence run exhausted a day between them. Pro raises it to 200,000/day and meters per account rather than per IP |
| `CLASSIFY_API_KEY` is not usable: `keep/.secrets/classify-api-key.age` is 279 KB and decrypts to a PNG (every other API-key secret is ~450-510 bytes), and the env var it produces is the 8-byte PNG magic header, truncated at the first NUL | Certain, present today | High | Re-encrypt the secret from the key text before P6; until then every call bills against the anonymous per-IP free tier. Blocks P0b |

...
- Revert residue: **the only phase with a data undo.** Code revert restores the enum, lint, and the old binary opens the index because the column still exists. Opening is not using it: the new writer omits `domain` from its INSERT (`vault/src/search/index.rs:291`), so rows indexed after P11 carry SQL NULL there, and the old `NoteRow.domain` is a non-optional `String` that fails to decode a NULL. After reverting the binary the operator runs `sb oracle index --force` with the OLD binary before retrieval is usable. The undo plan covers notes that existed at the obsidian anchor commit; notes created after it were never classified into `domain` and the rollback does not give them one. Frontmatter undo is the obsidian commit taken first; the inverse plan file is not exact (it also strips pre-existing same-name tags and cannot reconstruct `resources`/`system`). Ledger undo is the backup copy. The deployed `obsidian-classify.md` returns on the next `otto deploy` of the reverted binary.
...
Open defects from C1 (all three CLOSED in 0.15.1, see C6; the wording below is what C3 recorded at the time of the fold):
- Defect 1, borg sites: `borg/src/pipeline/atomic.rs:250` and `borg/src/pipeline/session.rs:644` still `truncate()`. The CLAUDE.md invariant holds at the cortex site and at `filter_and_cap` and `apply_retag`, and does not hold at these two. SHIPPED in 0.15.1.
- Defect 3: `borg/src/pipeline/atomic.rs:277-292` still caps every preserved list. SHIPPED in 0.15.1.
- Defect 4: `borg/src/pipeline/session/tests.rs:1194-1207` still re-implements the union. SHIPPED in 0.15.1.
...
### C6. The three borg defects, closed (0.15.1)

Redone from the C3 spec in a fresh branch, not folded from `fold-borg-wip` (that branch was never compiled or tested and stays a reference). Three source files, three test files, one crate plus one `vault` constant:

- Defect 1, both borg capping sites. `atomic::union_capped` and the session replace loop now cap through `vault::canonical::cap_protecting` instead of `truncate`, so a `no-classifier-tags` entry claims a slot ahead of an unprotected one. `apply_cortex_fields` takes `Option<&CanonicalSet>` in place of a bare `max_per_note`: the snapshot carries both the cap and the protect list, `None` means no vocabulary is loaded and nothing is capped, which is what the old `usize::MAX` sentinel meant. Shape copied from the cortex twin (b2d425d).
- Defect 3, per-list cap. `apply_cortex_fields` splits the merge: `tags` goes through `union_capped`, every other `CORTEX_PRESERVE_KEYS` list through the new uncapped `union`. `max-per-note` is a tags policy; `cortex-quality-issues` is no longer truncated at 8. `vault::schema::TAGS_KEY` names the key once for every writer that special-cases it.
- Defect 4, session union test. `session_replace_merges_tags` is split in two: `session_replace_captures_prior_tags_instead_of_carrying_them` keeps the `read_prior_frontmatter` assertion, and `session_replace_merges_prior_tags_into_the_published_note` drives `process_session_inner` end to end, writes cortex-added tags onto the landed note, replays the trace, and asserts the merged list read back off disk. AC3's test list is updated to the new name.

Break-the-code evidence, each regression applied and reverted:
- Session loop deleted: `session_replace_merges_prior_tags_into_the_published_note` fails, `left: []` vs `right: ["ai", "rust"]`.
- `cap_protecting` reverted to `truncate`: `protected_tag_in_the_fresh_set_survives_a_full_cap` and `protected_tag_in_the_preserved_set_survives_an_oversize_preserved_list` fail.
- The `tags`-only guard removed: `the_cap_applies_to_tags_only_and_not_to_the_other_preserved_lists` fails.

...
- **cluster failure** (LLM API error): if transient (429, 503, network), exponential backoff up to 3 retries within the tick; if permanent (4xx other than 429), set `last_failure_stage = 'cluster'` and skip this session. `sessions.last_cluster_offset` is NOT advanced. The same turns are re-clustered next tick. Because cluster output for already-clustered ranges is persisted in `cluster_assignments`, a failure here does not cause re-clustering of *previously* clustered ranges.

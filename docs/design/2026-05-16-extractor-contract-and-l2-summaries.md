...
#### Phase 9 - Deferred-items cleanup + verbatim preservation
**Model:** opus

The post-implementation architect audit identified three deferred items from the original phases plus one design-review-stage defect. All closed in five mergeable sub-phases. See [2026-05-16-extractor-contract-l2-phase-9-cleanup.md](2026-05-16-extractor-contract-l2-phase-9-cleanup.md).

- **9a** — Thread Phase-6 docstring rewrite + Stage-1 `transcript.md` persistence so `borg replay --from-stage 2` can re-run distillation against the staged input. Resolves L2 Open Question #1.
- **9b** — GitHub `fetch_repo` captures raw `response.bytes()` before deserialization and the staged `fetched.html` carries the `{"repo": ..., "readme": ...}` JSON envelope (`extractor: github-api`); rendered repo transcript lands as `transcript.md` (`extractor: github-render`). No `Serialize` derives added to internal structs.
- **9c-hotfix** — `Distilled.transcript: Option<String>` field added; `SUMMARY_CHAR_LIMIT = 280` deleted from `IdeaDistiller` and `PassthroughDistiller` (data-loss defect); renderer emits `## Transcript` body section when present; `DistillKind::Vocabulary` added (routes through `IdeaDistiller`); Idea-classified text and Vocabulary cut over to the distiller dispatcher. `IdeaDistiller` ID bumped to `distill-idea-v2`.
- **9c-image** — Real `ImageDistiller<F>` with single-call `distill-image` Fabric pattern. Vision+OCR concatenation preserved verbatim in `Distilled.transcript`. `process_image_inner` cut over.
- **9c-voicenote** — Real `VoiceNoteDistiller<F>` with map-reduce orchestration ported from `VideoDistiller` (timestamps stripped). Three patterns: `distill-voicenote`, `distill-voicenote-chunk`, `distill-voicenote-reduce`. Groq transcript preserved verbatim in `Distilled.transcript`. `process_audio_inner` cut over. `PassthroughDistiller` has zero consumers after this phase.

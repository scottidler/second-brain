...
2. **Phase 2 lands**: shared `distillers/` crate ships with `Idea` / `Image` / `VoiceNote` distillers. Borg's Stage 3 renders `Distilled` into the published file's body and frontmatter for those kinds. URL-based kinds still emit legacy `summary.md`. VaultWatcher reindexes the new structured notes via `index_vault` automatically.
3. **Phases 3-6 land per-kind**: as each distiller ships, the corresponding kind switches off the old path. Shipping order: article (largest volume) → YouTube (highest signal-per-dollar gain) → repo → thread.
